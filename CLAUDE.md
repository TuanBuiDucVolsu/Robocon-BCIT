...
| `docs/HAPPY_CASE.md` | **Bố cục kiện hàng tốt nhất khi TỰ ĐẶT được**. ⛔ Ràng buộc trong đó CHỈ đúng khi tự xếp — vòng chính thức BTC đặt 12 kiện NGẪU NHIÊN trên 3 giá nên **2 ô cùng tầng CÓ THỂ TRÙNG LOẠI**; đừng suy ngược thành luật trong code (đã xảy ra 04/08, ép sai kiện đang nhận đúng). Khi tự xếp thì mỗi cặp là 2 nhà máy KHÁC nhau → 6 cặp = 6 cạnh đồ thị đủ 4 đỉnh, chỉ còn tự do việc gán vào 6 tầng (đã duyệt hết 720 cách). Cách xếp chỉ đáng ~20 điểm; thứ thật sự chặn là NGÂN SÁCH 240s |
...
| `tests/test_smoke.py` | 8 smoke tích hợp trên sa bàn. **HỎI nửa sân đầu phiên** (`_ask_board_side`) — đặt sai thứ tự nhà máy là lỗi KHÔNG có tín hiệu báo. **8 = đường dài KHÔNG bốc hàng** (xuất phát → trước Kệ 3 → lùi → Samsung): tách điều hướng khỏi lift/camera, vì bốc hàng hỏng thì mọi bài có pickup đều dừng trước khi chạy tới phần điều hướng |
