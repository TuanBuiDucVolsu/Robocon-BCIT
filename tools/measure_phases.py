...
SPANS = [
    ...
    Span("lift_pickup", "Bốc hàng (nâng→luồn→nhấc)",
         start=r"Bốc hàng tầng \d+ — lần",
         end=r"Xác nhận: (CẢ 2 pallet trên càng|có pallet trên càng)",
         abort=(r"Bốc hàng THẤT BẠI sau \d+ lần",)),
    Span("insert", "Luồn càng vào pallet (IR dẫn)",
         start=r"Luồn càng: tiến \d+%",
         end=r"Luồn càng: điều kiện đạt sau",
         abort=(r"Luồn càng: (đã tới|hết) ",)),
...
ANOMALIES = [
    ...
    ("Luồn càng hỏng",             r"không luồn được càng vào pallet"),
    ("Bốc hàng phải thử lại",      r"Lần \d+: bốc hàng chưa thành công"),
