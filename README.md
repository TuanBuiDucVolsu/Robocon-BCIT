...
Nhận diện kiện hàng bằng **so khớp đặc trưng ORB** (hình dạng/hoạ tiết decal),
dự phòng bằng **phân tích màu HSV** (không cần AI model, không cần huấn luyện).
...
| Camera AI | Camera CSI + OpenCV (ORB + HSV) | Bắt buộc (bảng O2) |
...
│   ├── vision.py          #   Camera CSI + classify_pair() — ORB chính, HSV dự phòng
│   ├── shape_match.py     #   ShapeMatcher: ORB + BFMatcher + RANSAC homography
│   └── templates/         #   Ảnh mẫu 4 kiện hàng (tools/capture_templates.py tạo ra)
│
...
│   ├── calibrate_vision.py   #   Chốt COLOR_RANGES từ ảnh thật 4 kiện hàng (HSV dự phòng)
│   ├── capture_templates.py  #   Chụp ảnh mẫu 4 kiện hàng cho nhận diện ORB (chính)
